...
bool wrong_color(int seedNumLeft) {
    return (side && seedNumLeft < 0) || (!side && seedNumLeft > 0);
}
...
void loop() {
    ...
    case ON_CIRCLE:
        ...
        case 100: // Probably stuck, back up
            ...
            if((rs>RSGWThreshold)&&(rc>RCGWThreshold)){//START PATRICK MOD--adds turning to backing up if both see grey
                while(!((rs>RSGWThreshold)&&(rc<RCGWThreshold))){ 
                    bot.move(0,50);
                    rs = bot.getSideReflect();
                    rc = bot.getCenterReflect();
                }
            } //END PATRICK MOD
            break;
    ...
    case FIND_BOX:///PATRICK MOD START
        ...
        case 0:
        {
            ...
            int seedNum = bot.readGC(MudduinoBot::FRONT);
